...
from .base_initialiser import BaseInit
...
class Encoder(nn.Module):
    def __init__(self,domain):
        ...
        self.mean_head = nn.Linear(self.hyper['hidden_units'],self.hyper['latent_size'])
        self.lstd_head = nn.Linear(self.hyper['hidden_units'],self.hyper['latent_size'])
...
class Decoder(nn.Module):
    def __init__(self,domain):
        ...
        self.series_cont_mean = nn.Linear(self.hyper['hidden_units'],self.domain.con_out_dim)
        self.series_cont_lstd = nn.Linear(self.hyper['hidden_units'],self.domain.con_out_dim)
        self.series_bin = nn.Linear(self.hyper['hidden_units'],self.domain.bin_out_dim)
        self.static_cont_mean = nn.Linear(self.hyper['hidden_units'],self.domain.static_con_dim)
        self.static_cont_lstd = nn.Linear(self.hyper['hidden_units'],self.domain.static_con_dim)
        self.static_bin = nn.Linear(self.hyper['hidden_units'],self.domain.static_bin_dim)
...
class VAE(nn.Module):
    ...
    def loss(self,batch):
        ...
        series_mean,series_lstd,series_prob,static_mean,static_lstd,static_prob = self.decoder(latents)

        series_cont_dist = torch.distributions.normal.Normal(series_mean,torch.exp(series_lstd))
        series_cont_log_l = series_cont_dist.log_prob(x_series[:,0,:][:,:self.domain.con_out_dim])

        series_bin_dist = torch.distributions.bernoulli.Bernoulli(series_prob)
        series_bin_log_l = series_bin_dist.log_prob(x_series[:,0,:][:, \
                            self.domain.con_out_dim:(self.domain.con_out_dim+self.domain.bin_out_dim)])

        static_cont_dist = torch.distributions.normal.Normal(static_mean,torch.exp(static_lstd))
        static_cont_log_l = static_cont_dist.log_prob(x_static[:,:self.domain.static_con_dim])

        static_bin_dist = torch.distributions.bernoulli.Bernoulli(static_prob)
        static_bin_log_l = static_bin_dist.log_prob(x_static[:, \
                        self.domain.static_con_dim:(self.domain.static_con_dim+self.domain.static_bin_dim)])

        ll_loss = series_cont_log_l.sum() + series_bin_log_l.sum() + \
...
class VAEInit(BaseInit):
    ...
    def sample(self):
        ...
        series_mean,series_lstd,series_prob,static_mean,static_lstd,static_prob = self.model.decoder(latent)
        ...
        init_obs[:,:,self.domain.con_out_dim:self.domain.con_out_dim+self.domain.bin_out_dim] = series_bin

        static_obs[:,:self.domain.static_con_dim] = static_cont
        static_obs[:,self.domain.static_con_dim:self.domain.static_con_dim+self.domain.static_bin_dim] \
                = static_bin
